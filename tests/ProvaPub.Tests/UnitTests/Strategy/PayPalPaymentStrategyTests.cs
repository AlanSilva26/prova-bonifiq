...
{
    private readonly PayPalPaymentStrategy _paymentStrategy;
    private readonly Mock<IOrderRepository> _orderRepositoryMock;

    public PayPalPaymentStrategyTests()
    {
        _orderRepositoryMock = new Mock<IOrderRepository>();
        _paymentStrategy = new PayPalPaymentStrategy(_orderRepositoryMock.Object);
    }

    [Fact(DisplayName = "Deve criar um pedido corretamente ao processar pagamento via PayPal")]
    public async Task PayOrderAsync_ShouldCreateOrderSuccessfully()
    {
        int customerId = 1;
        decimal paymentValue = 250.00m;
        var order = new Order
        {
